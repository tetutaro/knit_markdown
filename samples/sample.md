...
#. よっつめ
	#. よっつめのひとつめ
		#. よっつめのひとつめのひとつめ
			#. よっつめのひとつめのひとつめのひとつめ
			#. よっつめのひとつめのひとつめのふたつめ
2. マーカーとして数字を直接指定した場合（この場合は数字は保持されない）
	7) 半括弧のマーカーもできる（数字が保持される）
		(9) 両カッコのマーカーもできる（数字が保持される）
			ii. アラビア数字のもできる（数字が保持される）
c. アルファベットの場合（文字が保持される）
	g) 大文字は認識されないが、括弧は認識される
		(k) アルファベットと数字で形式が異なるので、別リストと認識される
#. 直前のマーカーが保持される
...
| テーブルの種類 | 書き方 | メリット | デメリット |
| :---- | :---: | ---: | --- |
| シンプルテーブル | ヘッダの下にハイフン | 簡単 | 巾を合わせる必要 |
| マルチラインテーブル | 最初と最後にハイフン | 複数行 | 巾を合わせる必要 |
| グリッドテーブル | カラム間にバイプ | 巾を合わせやすい | アラインを設定できない |
| パイプテーブル | カラム間のバイプだけ | アラインもできる | 複数行できない |
| おすすめは | パイプテーブル | 簡単だし| 分かりやすい |

: テーブルの種類（キャプションはコロンから始まる行）{#tbl:sample1}
...
クラスを付けたレベル３のセクションが始まってから、次の何らかのセクションが指定されるまでがブロックになる
...
exapleblockもあるが、こちらはヘッダに自動的にカウンタが付く方。
